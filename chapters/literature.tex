...
\section{Review of Paper 3: Website Phishing Attack Detection Using Innovative Meta Learning-Based Ensemble Approach}
	...
	Naseeb et al. (2025) proposed a novel stacked ensemble model combining Artificial Neural Networks (ANN) and Bagging K-Nearest Neighbors (KNN) with Logistic Regression as a meta-learner classifier. Their hybrid framework leveraged ANN’s capability for global non-linear pattern identification and Bagging KNN’s proficiency in local pattern detection. The system incorporated SMOTE oversampling for class imbalance handling and employed k-fold cross-validation for robust performance validation using 2019--2024 datasets~\cite{ref3}.
	
	\subsubsection{Strengths and Contributions}
	\begin{itemize}
	...
	\subsubsection{Limitations and Gaps}
	\begin{itemize}
...
\section{Comparative Analysis}
	...
	Table~\ref{tab:comparative_analysis} provides a comparative summary of the reviewed literature, highlighting the techniques employed, performance metrics, identified gaps, and relevance to the CyberSentinel project.
	
	\begin{table}[h!]
		\centering
		\caption{Comparative Analysis of Reviewed Literature}
		\label{tab:comparative_analysis}
		\begin{tabular}{|p{3cm}|p{3cm}|p{2cm}|p{3.5cm}|p{3.5cm}|}
			\hline
			\textbf{Author \& Year} & \textbf{Technique} & \textbf{Accuracy} & \textbf{Key Gaps} & \textbf{Relevance to CyberSentinel} \\
			\hline
			Leand Thaci et al. (2024) & Random Forest, SVM, kNN in Chrome extension & Not reported & Limited cross-platform support; no zero-day attack evaluation & Demonstrates browser integration feasibility and real-time detection approach \\
			\hline
			R. Alazaidah et al. (2024) & Comparison of 24 classifiers (RF, J-48, Bayesian methods) & RF: 95.8\% & Small datasets; no adversarial robustness testing & Validates Random Forest effectiveness for phishing detection tasks \\
			\hline
			Saba Naseeb et al. (2025) & Stacked ensemble (ANN + Bagging KNN + Logistic Regression) & 97.20\% & High computational cost; limited deep learning comparison & Provides ensemble methodology and SMOTE-based imbalance handling strategy \\
			\hline
		\end{tabular}
	\end{table}
	
	The comparative analysis reveals a progressive evolution from single-algorithm implementations to sophisticated ensemble approaches, with accuracy improvements from approximately 96\% to 97.20\%. However, common gaps persist across all reviewed works, particularly concerning adversarial robustness, computational efficiency, and cross-platform deployment. CyberSentinel addresses these limitations by integrating ensemble methods with optimized real-time processing and comprehensive threat intelligence.
