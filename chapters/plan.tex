...
The CyberSentinel project follows a structured 16-week development timeline divided into four distinct phases. Table~\ref{tab:project-phases} outlines the major phases and their respective durations.
...
The frontend development has been successfully completed, delivering a professional, responsive web application. Figure~\ref{fig:url-analysis-interface} shows the main URL analysis interface.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/reporturl.jpg}
	\caption{CyberSentinel URL Analysis Interface}
	\label{fig:url-analysis-interface}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/dashboard.jpeg}
	\caption{CyberSentinel URL Analysis Interface}
	\label{fig:url-analysis-interface-2}
\end{figure}
...
\begin{figure}[htbp]
	...
	\caption{CyberSentinel URL Analysis Interface}
	\label{fig:url-analysis-interface-3}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/home.jpg}
	\caption{CyberSentinel URL Analysis Interface}
	\label{fig:url-analysis-interface-4}
\end{figure}
...
The community feedback interface has been implemented to enable continuous model improvement. Figure~\ref{fig:community-feedback} demonstrates the user-friendly reporting system.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/reporturl.jpg}
	\caption{Community Feedback System Interface}
	\label{fig:community-feedback}
\end{figure}
...
\begin{itemize}
	\item Accuracy Target: 95\% (Achieved: 98\%)
	\item Response Time Target: $<$100ms (Achieved: 78ms average)
	\item F1-Score Target: 0.90 (Achieved: 0.965)
	\item Category Coverage: 4 threat categories (Benign, Defacement, Malware, Phishing)
	\item Cross-platform Compatibility: Responsive design across all devices
\end{itemize}
...
The training dataset comprises 651,191 URLs distributed across four distinct threat categories. Figure~\ref{fig:url-distribution} illustrates the distribution of URL types in the training dataset.
...
Figure~\ref{fig:training-progress} presents the detailed training progression showing loss reduction, gradient norm optimization, and learning rate scheduling throughout the training process.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/model_during_trainning.png}
...
The CyberSentinel model achieved exceptional performance across all standard evaluation metrics. Figure~\ref{fig:performance-evaluation} presents the comprehensive evaluation results.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/evaluation_results.jpg}
...
The CyberSentinel model demonstrates strong performance across all evaluation metrics including accuracy, precision, recall, and F1-score. These metrics indicate the effectiveness of the system in correctly identifying both malicious and benign URLs.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/performance_metrics.png}
	\caption{Model Performance Metrics (Accuracy, Precision, Recall, F1-Score)}
	\label{fig:performance-metrics}
\end{figure}

The model achieves consistently high values across all metrics, showing balanced performance. High precision ensures fewer false positives, while strong recall indicates effective detection of malicious URLs. The F1-score confirms the overall reliability of the system.

\section{System Feature Comparison}

A comparative analysis between traditional URL detection systems and CyberSentinel highlights the improvements introduced by the proposed system.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/feature_comparison.png}
	\caption{Comparison of Features: Traditional Systems vs CyberSentinel}
	\label{fig:feature-comparison}
\end{figure}

The comparison clearly shows that CyberSentinel supports advanced features such as real-time detection, machine learning integration, feedback learning, and QR code analysis, which are either absent or limited in traditional systems. This demonstrates the superiority of the proposed approach in terms of functionality and adaptability.

\section{Category-wise Performance Analysis}

\subsection{Detailed Threat Category Performance}

Figure~\ref{fig:category-performance} presents the detailed performance analysis across all four threat categories, demonstrating consistent excellence in classification accuracy.
...
\textbf{Benign URL Detection:} The model demonstrates exceptional accuracy (98\% precision, 99\% recall) in identifying legitimate URLs, minimizing false positive rates that could impact user experience.

\textbf{Defacement Detection:} Equally impressive performance (98\% precision, 99\% recall) in detecting website defacement attacks, which are often challenging to identify due to subtle modifications.

\textbf{Phishing Detection:} Strong performance (93\% precision, 94\% recall) in identifying phishing attempts, which represent one of the most sophisticated and evolving threat categories.

\textbf{Malware Detection:} Excellent results (95\% precision, 96\% recall) in detecting malware distribution URLs, critical for preventing malicious software installations.

\subsection{Performance Comparison with Existing Systems}

Table~\ref{tab:system-comparison} presents a comparison of CyberSentinel performance against existing URL threat detection systems documented in literature.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lclc}
		...
		\midrule
		CyberSentinel & 98.0\% & Fine-tuned LLM & 4 categories \\
...
\begin{itemize}
	\item 15\% improvement over best-performing traditional machine learning approaches
	\item 30\% improvement over rule-based heuristic systems
	\item Comprehensive categorization providing 4-category classification vs.\ binary classification
	\item Adaptive learning capability through community feedback integration
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Hybrid Attacks:} Complex multi-stage attacks with legitimate initial URL
\end{itemize}
